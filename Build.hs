...
build = do
    ...
    header     <- IO.readFile "template/header.html"
    footer     <- IO.readFile "template/footer.html"
    ...
    let catFiles = map (categoryFile header footer) catList
    ...
    IO.writeFile "website/archive.html" $ Text.concat 
      [header, archive catList meta, footer]
    ...
    IO.writeFile "website/index.html" 
      $ Text.concat [header, Html.index $ take postsOnHome meta, footer]


-------------------------------------------------------------------------------
-- | 'getPostMeta' parses a post's metadata from the markdown file
...
addPostHtml :: PostMeta -> IO ()
addPostHtml post = do
    content <- IO.readFile path
    IO.writeFile path $ postHtml False post
    where
...
categoryFile :: Text.Text -> Text.Text -> (Text.Text, [PostMeta]) 
                -> (Text.Text, Text.Text)
categoryFile header footer cat = (Text.concat ["categories/", fst cat, ".html"]
  , Text.concat [header, categoryIndex cat, footer])
